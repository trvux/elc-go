...
- `cmd/sync-ai-pricing` (mới): với mỗi `ai_providers.pricing_doc_url` đã set, fetch trang giá, đưa nội dung cho model `role=chat` đầu tiên đang active (không dùng `role=classifier`: classifier optional/fail-open theo thiết kế §1 điểm guardrail, nên có thể chưa được cấu hình — role=chat luôn bắt buộc có ít nhất 1 model active để chat hoạt động, nên chắc chắn có model để dùng) trích xuất JSON đúng shape `pricing` ở trên, validate rồi `UPDATE ai_models.pricing` cho các `model_name` đã khớp sẵn trong DB. Model xuất hiện trong docs nhưng chưa có row trong `ai_models` thì chỉ log cảnh báo, **không tự tạo** — tránh tool tự ý đẩy model chưa duyệt vào fallback chain. Chạy định kỳ qua GitHub Actions cron (`.github/workflows/sync-ai-pricing.yml`, theo mẫu `.github/workflows/migrate-images.yml`), 1 lần/ngày (03:00 UTC).
