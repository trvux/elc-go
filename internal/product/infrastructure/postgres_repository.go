...
const productColumns = `p.id, p.category_id, p.brand_id, p.name, p.slug,
	...
	p.condition,
	p.meta_title, p.meta_description, p.seo,
...
const plainProductColumns = `id, category_id, brand_id, name, slug, description, specs, normalized_specs,
	images, labels,
...
func buildOrderBy(filter domain.ProductFilter) string {
	...
	case domain.SortByPriceAsc:
		return "p.display_price ASC"
	case domain.SortByPriceDesc:
		return "p.display_price DESC"
...
func buildFilterConditions(filter domain.ProductFilter, exclude filterExclude, fuzzy bool) ([]string, []any) {
	...
	// (search_vector, name+variant_mpns via 'simple' config + unaccent) — the
...
func (r *PostgresProductRepository) resolveSearchMode(ctx context.Context, filter domain.ProductFilter) (fuzzy bool, err error) {
	...
	query := "SELECT EXISTS(SELECT 1 " + productJoin
	if len(conditions) > 0 {
...
func (r *PostgresProductRepository) queryProducts(ctx context.Context, filter domain.ProductFilter, fuzzy bool) ([]*domain.ProductWithRelations, error) {
	...
	query := "SELECT " + productColumns + ", " + rankExpr + " " + productJoin
	if len(conditions) > 0 {
		query += " WHERE " + strings.Join(conditions, " AND ")
	}
...
func (r *PostgresProductRepository) brandFacets(ctx context.Context, filter domain.ProductFilter, fuzzy bool) ([]domain.BrandFacet, error) {
	...
	query := "SELECT DISTINCT br.id, br.name, br.slug " + productJoin +
...
func (r *PostgresProductRepository) specFacets(ctx context.Context, filter domain.ProductFilter, fuzzy bool) ([]domain.SpecFacet, error) {
	...
	query := "SELECT unnest(p.normalized_specs) AS facet " + productJoin
	if len(conditions) > 0 {
		query += " WHERE " + strings.Join(conditions, " AND ")
	}
...
func (r *PostgresProductRepository) priceFacets(ctx context.Context, filter domain.ProductFilter, fuzzy bool) (int64, int64, error) {
	...
	query := "SELECT MIN(p.display_price), MAX(p.display_price) " + productJoin
	if len(conditions) > 0 {
		query += " WHERE " + strings.Join(conditions, " AND ")
	}
...
func (r *PostgresProductRepository) Create(ctx context.Context, product *domain.Product, tagIDs []string, options []domain.ProductOptionInput, variants []domain.ProductVariantInput, attributeValues []domain.ProductAttributeValueInput) (*domain.Product, error) {
	...
	query := `
		INSERT INTO products (
			category_id, brand_id, name, slug, description, specs, normalized_specs,
			images, labels,
			is_featured, is_published, order_index, condition,
			meta_title, meta_description, seo,
			product_line_id, short_description, warranty_months, warranty_terms
	...
	row := tx.QueryRow(ctx, query,
		...
		product.MetaTitle(), product.MetaDescription(), seoJSON,
...
func (r *PostgresProductRepository) Update(ctx context.Context, product *domain.Product, tagIDs *[]string, options *[]domain.ProductOptionInput, variants *[]domain.ProductVariantInput, attributeValues *[]domain.ProductAttributeValueInput) (*domain.Product, error) {
	...
	query := `
		...
		SET category_id = $1, brand_id = $2, name = $3, slug = $4,
			description = $5, specs = $6, normalized_specs = $7,
			images = $8, labels = $9,
			is_featured = $10, is_published = $11, order_index = $12, condition = $13,
			meta_title = $14, meta_description = $15, seo = $16,
			product_line_id = $17, short_description = $18, warranty_months = $19, warranty_terms = $20
		WHERE id = $21
		RETURNING ` + plainProductColumns
...
func scanProduct(row rowScanner) (*domain.Product, error) {
	...
	if err := row.Scan(
		...
		&condition,
		&metaTitle, &metaDescription, &seoRaw,
