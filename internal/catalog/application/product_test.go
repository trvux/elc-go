...
func TestCreateProduct_NormalizesPriceAtWriteTime(t *testing.T) {
	repo := newFakeProductRepository()
	ctx := context.Background()

	input := baseCreateInput()
	input.OriginalPrice = 100000
	input.DiscountPercent = 10

	p, err := CreateProduct(ctx, repo, input)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
...
func TestUpdateProduct_PartialUpdate(t *testing.T) {
	...
	// SKU wasn't part of the input, must stay unchanged.
	if updated.SKU() != created.SKU() {
		t.Errorf("expected sku to remain %s, got %s", created.SKU(), updated.SKU())
	}
}
...
func TestUpdateProduct_PricingResolvedTogether(t *testing.T) {
	repo := newFakeProductRepository()
	ctx := context.Background()

	input := baseCreateInput()
	input.OriginalPrice = 100000
	created, _ := CreateProduct(ctx, repo, input)

	// Updating only discountPercent must still produce a consistent sale
	// price — same invariant as service's UpdatePricing.
	newDiscount := 20.0
	updated, err := UpdateProduct(ctx, repo, domain.UpdateProductInput{ID: created.ID(), DiscountPercent: &newDiscount})
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if updated.SalePrice() == nil || *updated.SalePrice() != 80000 {
		t.Errorf("expected sale price 80000, got %v", updated.SalePrice())
	}
}
